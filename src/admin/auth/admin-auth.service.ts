  ...
  async login(loginDto: AdminLoginDto, ip: string) {
    ...
    if (!user || !(await bcrypt.compare(password, user.password))) {
      ...
      if (fails >= 4) { // 5th attempt fails
        ...
        throw new ForbiddenException('Too many login attempts. Account blocked for 15 minutes.');
      } else {
    ...
    // Generate Token
    const payload = { id: user.id, email: user.email, role: user.role, isAdmin: true };
